...
public class EditBuyerCommandParser implements Parser<EditBuyerCommand> {
    ...
    public EditBuyerCommand parse(String args) throws ParseException {
        ...
        ArgumentMultimap argMultimap =
                ArgumentTokenizer.tokenize(args, PREFIX_NAME_TO_EDIT, PREFIX_NAME, PREFIX_PHONE, PREFIX_EMAIL,
                        PREFIX_HOUSING_TYPE, PREFIX_BUDGET);

        if (!arePrefixesPresent(argMultimap, PREFIX_NAME_TO_EDIT)) {
            throw new ParseException(String.format(MESSAGE_INVALID_COMMAND_FORMAT, EditBuyerCommand.MESSAGE_USAGE));
        }

        argMultimap.verifyNoDuplicatePrefixesFor(PREFIX_NAME_TO_EDIT, PREFIX_NAME, PREFIX_PHONE, PREFIX_EMAIL,
                PREFIX_HOUSING_TYPE, PREFIX_BUDGET);

        Name name = ParserUtil.parseName(argMultimap.getValue(PREFIX_NAME_TO_EDIT).get());
        ...
        return new EditBuyerCommand(name, editBuyerDescriptor);
    }
