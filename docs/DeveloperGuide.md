...
### B.10 Fixing Index Error Message

#### B.10.1 Motivation
- In the current implementation, the commands that uses `INDEX`, such as `editBuyer`, `editSeller`, `delete` and `view`,
shows a wrong error message when the `INDEX` exceeds the maximum int value. It would show an error message for negative 
